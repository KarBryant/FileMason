...
class Reader:
    """
    Reads files from a directory and returns FileItem objects.

    Skips hidden files, symlinks, subdirectories, and tracks skipped items with reasons.
    """
 
    def read_directory(self, directory:Path | str) -> tuple[list[FileItem], list[tuple[Path,str]]]:
        """
    ...
    def _create_file(self,file: Path) -> FileItem:
        ...
        file_created = datetime.fromtimestamp(stats.st_ctime,
                    tz=timezone.utc).replace(microsecond=0)
        return FileItem(name=file_name,
                        path=file_path,
