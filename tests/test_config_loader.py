...
from app.exceptions import ConfigValidationError, ConfigFileError, ConfigParseError
...
def test_config_with_no_buckets(config_with_no_buckets,monkeypatch):
    ...
    monkeypatch.setattr(config_loader,"config_path", config_with_no_buckets,raising=True)
...
def test_load_config_multiple_extensions(config_with_multiple_extensions,monkeypatch):
    config_loader._config_cache = None
    monkeypatch.setattr(config_loader,"config_path", config_with_multiple_extensions, raising=True)
...
def test_load_config_with_empty_bucket(config_with_empty_bucket,monkeypatch):
    config_loader._config_cache = None
    monkeypatch.setattr(config_loader,"config_path", config_with_empty_bucket,raising=True)

    with pytest.raises(ConfigValidationError):
        load_config()

def test_load_config_with_many_empty_buckets(config_with_many_empty_buckets,monkeypatch):
    config_loader._config_cache = None
    monkeypatch.setattr(config_loader, "config_path", config_with_many_empty_buckets, raising=True)
