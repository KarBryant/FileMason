...
def load_config() -> Dict[str,Any]:
    ...
    try:
        ...
        raise ConfigFileError(f"Insufficient permissions to read {config_path}: {e}") from e
    except FileNotFoundError as e:
        raise ConfigFileError(f"Config file cannot be found at {config_path}: {e}") from e
    except OSError as e:
...
def _validate_bucket_config(buckets:Dict[str,List[str]]) -> None:
    ...
    if empty_buckets:
        if len(empty_buckets) == 1:
            ...
            raise ConfigValidationError(f"Buckets {empty_buckets_list} cannot be empty.")

    if duplicate_extensions:
        duplicate_details = '\n'.join(f"- {extension} found in {', '.join(bucket_list)}" for extension, bucket_list in duplicate_extensions.items())
        raise ConfigValidationError(f"Duplicate extensions detected: \n {duplicate_details}")
